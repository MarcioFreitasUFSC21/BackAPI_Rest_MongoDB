    ...
    delete: async (req, res, next) => {
        try {
            ...
            if(!service){ // the validation service test 
                res.status(404).json({deleteService, msg: "Le service n'est pas retornee avec success!"});
                return;
                }
                const deleteService = await serviceModel.findByIdAndDelete(id);
                res.status(200).json({deleteService, msg: "L'archive n'est excludee avec success!"});
